...
import { handleCrash } from "./internal/main";
...
export function assert<T>(cond: T, msg: string) {
  if (!cond || cond === null || cond === undefined) fatal(msg || "Assertion Failed");
  return cond as Exclude<T, null | undefined>;
}
...
/// msg: string
///   The error message to show if the condition is false.
export function assertDebug(cond: unknown, msg: string) {
  if (!cond) {
    if (CONFIG.DEBUG) {
...
export function assertEquals(expected: unknown, actual: unknown, what: string) {
  if (expected !== actual) {
...
export function checkType(varName: string, varValue: unknown, varType: string) {
...
export function checkNumber(varName: string, varValue: number, optMin?: number, optMax?: number) {
...
/// varValue: any
///   The value of the variable.
export function checkString(varName: string, varValue: string) { return checkType(varName, varValue, "string"); }
...
/// varValue: any
///   The value of the variable.
export function checkBoolean(varName: string, varValue: boolean) { return checkType(varName, varValue, "boolean"); }
...
/// varValue: any
///   The value of the variable.
export function checkFunction(varName: string, varValue: Function) { return checkType(varName, varValue, "function"); }
...
/// varValue: any
///   The value of the variable.
export function checkObject(varName: string, varValue: Object) {
  if (varValue === null) {
...
///   with the class name.
export function checkInstanceOf(varName: string, varValue: unknown, expectedClass: unknown) {
  // @ts-expect-error
...
/// varValue: any
///   The value of the variable.
export function checkArray(varName: string, varValue: Array<unknown>) {
...
export function loadFileAsync(url: string) {
...
export function randomInt(lowInclusive: number, highInclusive: number) {
...
///   the array is empty.
export function randomPick(array: unknown[]) {
...
export function intersectIntervals(as: number, ae: number, bs: number, be: number, result?: IntersectInterval) {
  checkNumber("as", as);
  checkNumber("ae", ae);
  checkNumber("bs", bs);
  checkNumber("be", be);
...
const BLANK_INTERVAL: IntersectInterval = { start: 0, end: 0 };
export function intersectRects(r1: Rectangle, r2: Rectangle, dx1 = 0, dy1 = 0, dx2 = 0, dy2 = 0, result?: Rectangle) {
  checkObject("r1", r1);
  checkObject("r2", r2);
  ...
  const xint = { ...BLANK_INTERVAL };
  const yint = { ...BLANK_INTERVAL };
