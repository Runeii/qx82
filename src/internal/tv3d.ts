import { ClampToEdgeWrapping, Color, LinearFilter, LinearMipmapLinearFilter, PerspectiveCamera, PlaneGeometry, Scene, ShaderMaterial, UVMapping, Vector2, Vector3, WebGLRenderer } from "three";
...
let lastCanvasSize = new Vector2();
let scene: Scene;
...
export function setup(canvasToRenderTo: HTMLCanvasElement, canvasWithScreenContents: HTMLCanvasElement) {
  ...
  canvasTexture = new CanvasTexture(texCanvas, UVMapping, ClampToEdgeWrapping, ClampToEdgeWrapping, LinearFilter, LinearMipmapLinearFilter);

  const geomWidthSegments =
    Math.round(
      (CONFIG.SCREEN_COLS / CONFIG.SCREEN_ROWS) * GEOM_HEIGHT_SEGMENTS);
  const geom = new PlaneGeometry(1, 1, geomWidthSegments, GEOM_HEIGHT_SEGMENTS);
  deformPlane(geom);
  screenMat = new ShaderMaterial({
    uniforms: {
      tex: { value: canvasTexture },
...
// non-null.
export function getDesiredCanvasSize(targetCanvas: HTMLCanvasElement | null = null) {
...
function deformPlane(geom: PlaneGeometry) {
