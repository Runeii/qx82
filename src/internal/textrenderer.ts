...
import chrFile from '../assets/chr.png';
...
export class TextRendererFont {
  ...
  getImageForColor(colorNumber: number) { return this.chrImages_[colorNumber]; }
  ...
  regenColors() {
    ...
    if (!this.origImg_) {
      qut.fatal("TextRendererFont.regenColors(): origImg_ is null.");
      return
    }
    ...
    if (!ctx) {
      qut.fatal("TextRendererFont.regenColors(): could not get 2D context.");
      return
    }
    this.chrImages_ = [];
...
export class TextRenderer {
  fonts_: { [fontName: string]: TextRendererFont };
  ...
  constructor() {
    ...
    this.curFont_ = null;
    this.origFgColor_ = null;
    this.origBgColor_ = null;
  }
  ...
  async loadFontAsync(fontName: string, fontImageFile: string) {
    qut.checkString("fontName", fontName);
    qut.checkString("fontImageFile", fontImageFile);
  ...
  print(text: string) {
    ...
    if (this.curFont_ === null) {
      qut.fatal("TextRenderer.print(): curFont_ is null.");
      return;
    }
  ...
  printCentered(text: string, width: number) {
    qut.checkString("text", text);
  ...
  spr(ch: number, x: number, y: number) {
    qut.checkNumber("ch", ch);
    qut.checkNumber("x", x);
    qut.checkNumber("y", y);
  ...
  drawText(x: number, y: number, text: string, fontName?: string) {
    qut.checkNumber("x", x);
    qut.checkNumber("y", y);
    qut.checkString("text", text);
  ...
  printRect(width: number, height: number, ch: number) {
    qut.checkNumber("width", width);
  ...
  printBox(width: number, height: number, fill: boolean, borderCh: number) {
  ...
  put_(ch: number, col: number, row: number, fgColor: number, bgColor: number) {
  ...
  putxy_(ch: number, x: number, y: number, fgColor: number, bgColor: number, passedFont?: TextRendererFont) {
    const font = passedFont || this.curFont_;
    if (!font) {
      qut.fatal("TextRenderer.putxy_: font is null.");
      return;
    }
  ...
  processEscapeSeq_(text: string, startPos: number, pretend = false) {
    // Shorthand.
  ...
  runEscapeCommand_(command: string) {
